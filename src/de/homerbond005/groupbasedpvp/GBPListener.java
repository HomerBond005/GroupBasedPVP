...
public class GBPListener implements Listener {
	private final String stringCannotBeAttacked;
	private final String stringNoPermAttackAnyone;
	private final String stringGroupNoPermAttackAnyone;
	private final String stringGroup1NoPermAttackGroup2;
	...
	private boolean handleDamagerGroups(Player damager, Player damaged) {
		...
		if (damaged.hasPermission("GroupBasedPVP.pvp.protect")) {
		...
		for (int i = 0; i < lengthofdamager; i++) {
			...
			for (String disallowedGroup : disallowedGroups) {
				...
				if (illegal == true) {
					if (!damager.hasPermission("GroupBasedPVP.pvpgroup." + disallowedGroup)) {
						try {
							damager.setHealth(((Damageable) damager).getHealth() + penalty);
						} catch (IllegalArgumentException exe) {
							damager.setHealth(0D);
